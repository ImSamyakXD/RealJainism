        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {channels.map((ch, i) => (
            <a
              ...
              className={`reveal reveal-delay-${(i % 4) + 1} block bg-gradient-to-br ${ch.color} border border-border rounded-2xl p-6 card-hover group`}
            >
              ...
              <div className="mt-4 flex items-center gap-1 text-xs font-black uppercase tracking-widest text-primary group-hover:text-accent transition-colors">
                {ch.badge} <Icon name="ArrowRightIcon" size={12} />
              </div>
            </a>
          ))}
        </div>
